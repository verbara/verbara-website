...
const REQUIRED_KEYS = ['platform_version', 'image_ref', 'manifest_list_digest', 'released_at'];
const VERSION_RE = /^v\d+\.\d+\.\d+$/;
const DIGEST_RE = /^sha256:[0-9a-f]{64}$/;
const IMAGE_REF_RE = /^ghcr\.io\/verbara\/platform\/(api|realtime):(.+)$/;
...
if (errors.length) {
  console.error(`authorized-digests.json: ${errors.length} problem(s):`);
  for (const e of errors) console.error(`::error::${e}`);
  process.exit(1);
}

const n = (g) => (Array.isArray(doc[g]) ? doc[g].length : 0);
console.log(`authorized-digests.json OK — ${n('current')} current, ${n('deprecated')} deprecated, ${seenImageRefs.size} unique image refs.`);
